...
init_bap_rule() {
  if [[ -z ${BK_PUBLIC_REPO} ]]; then
    utils::log "ERROR" "init cncos-apiserver-proxy failed, empty BK_PUBLIC_REPO"
  else
    bap_image="${BK_PUBLIC_REPO}/blueking/cncos-apiserver-proxy:${APISERVER_PROXY_VERSION}"
  fi
  ...
  case "${CRI_TYPE,,}" in
    "docker")
      ...
      docker run -v "${PROXY_TOOL_PATH}":/tmp --rm --entrypoint /bin/cp "${bap_image}" \
        -f /data/cncos/cncos-apiserver-proxy/cncos-apiserver-proxy-tools /tmp/ || utils::log "ERROR" "pull ${bap_image} image failed"
      ;;
    "containerd")
      ...
      if ctr -n k8s.io i pull --hosts-dir "/etc/containerd/certs.d" "${bap_image}"; then
        if ! ctr -n k8s.io run --rm --mount type=bind,src="${PROXY_TOOL_PATH}",dst=/tmp,options=rbind:rw "${bap_image}" \
          bap-copy."$(date +%s)" /bin/cp -f /data/cncos/cncos-apiserver-proxy/cncos-apiserver-proxy-tools /tmp/; then
  ...
  "${PROXY_TOOL_PATH}"/cncos-apiserver-proxy-tools -cmd init -vs "${VIP}":"${VS_PORT}" -rs "${K8S_CTRL_IP}":6443 \
    -scheduler "${LVS_SCHEDULER}" -toolPath "${PROXY_TOOL_PATH}"/cncos-apiserver-proxy-tools
  "${ROOT_DIR}"/system/config_cncos_dns -u "${VIP}" k8s-api.cncos.local
  k8s::restart_kubelet
}
...
safe_source "${ROOT_DIR}/env/cncos.env"
...
if [[ "${ENABLE_APISERVER_HA}" == "true" ]]; then
  if [[ "${APISERVER_HA_MODE}" == "cncos-apiserver-proxy" ]]; then
    init_bap_rule
  else
    "${ROOT_DIR}"/system/config_cncos_dns -u "${VIP}" k8s-api.cncos.local
    k8s::restart_kubelet
