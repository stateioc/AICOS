...
ROOT_DIR="${SELF_DIR}/.."
CNCOS_ENV_FILE="${ROOT_DIR}/env/cncos.env"

readonly VERSION PROGRAM SELF_DIR ROOT_DIR CNCOS_ENV_FILE
...
Usage:
    $PROGRAM
      ...
      [ -c --config
        init: READ CLUSTER_ENV and init cncos.env file
        dump: print cncos.env file.
        clean: clean cncos.env file ]
EOF
  exit "$1"
}
...
init_env() {
  # host
  BK_HOME=${BK_HOME:-"/data/cncos"}
  K8S_IPv6_STATUS=${K8S_IPv6_STATUS:-"Disable"}
  LAN_IP=${LAN_IP:-}
  LAN_IPv6=${LAN_IPv6:-}
  CNCOS_SYSCTL=${CNCOS_SYSCTL:=1}
  ...
  ## if CNCOS_CP_WORKER==0, then untaint master
  CNCOS_CP_WORKER=${CNCOS_CP_WORKER:-0}
  ...
  APISERVER_HA_MODE=${APISERVER_HA_MODE:-"cncos-apiserver-proxy"}
  VIP=${VIP:-}
  ## cncos apiserver proxy
  ...
  PERSIST_DIR=${PERSIST_DIR:-"/root/.cncos"}
...
render_env() {
  utils::log "INFO" "RENDERING cncos env file"
  [[ -d "${ROOT_DIR}/env" ]] || install -dv "${ROOT_DIR}/env"
  cat >"${CNCOS_ENV_FILE}" <<EOF
# cncos config begin
## HOST
BK_HOME="${BK_HOME}"
LAN_IP="${LAN_IP}"
...
CNCOS_CP_WORKER="${CNCOS_CP_WORKER}"
...
config_init() {
  trap "utils::on_ERR;" ERR
  if [[ -f ${CNCOS_ENV_FILE} ]]; then
    #shellcheck source=/dev/null
    source "${CNCOS_ENV_FILE}"
  fi
  source_cluster_env
  init_env
  check_env
...
config_dump() {
  if [[ -f ${CNCOS_ENV_FILE} ]]; then
    ...
    utils::log "ERROR" "${CNCOS_ENV_FILE} not find!"
  fi
}

config_clean() {
  if [[ -f "${CNCOS_ENV_FILE}" ]]; then
    if grep -q "cncos config begin" "${CNCOS_ENV_FILE}"; then
      sed -ri.cncos-"$(date +%s)".bak "/cncos config begin/,/cncos config end/d" "${CNCOS_ENV_FILE}"
    fi
  fi
  utils::log "OK" "Clean ${CNCOS_ENV_FILE}"
}
