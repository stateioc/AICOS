...
# configure host for k8s-api.cncos.local and sysctl optimization
VERSION=0.1.0
PROGRAM="$(basename "$0")"
...
Usage:
    $PROGRAM
      ...
      [ -c --config
         dns: config cncos dns
         systcl: config cncos sysctl ]
      [ -d --del
	     dns: del cncos dns
		 systcl: del cncos sysctl ]
EOF
  exit "$1"
}
...
add_kernel_para() {
  ...
  sed -ri.cncos-"$(date +%s)".bak '/cncos config begin/,/cncos config end/d' /etc/sysctl.conf
...
add_limits() {
  ...
  cat >/etc/security/limits.d/99-cncos.conf <<EOF
# cncos config begin
*   soft  nproc    1028546
*   hard  nproc    1028546
*   soft  nofile    204800
...
add_master_hosts() {
  ...
  if [[ -z ${master_iplist[0]} ]]; then
    utils::log ERROR "CNCOS_K8S_CTRL_IP is null"
  fi
  "${ROOT_DIR}"/system/config_cncos_dns -u "${master_iplist[0]}" k8s-api.cncos.local
  return 0
}

add_mirror_hosts() {
  if [[ -n ${MIRROR_IP} ]]; then
    "${ROOT_DIR}"/system/config_cncos_dns -u "${MIRROR_IP}" mirrors.tencentyun.com
  fi
  return 0
}
...
del_dns() {
  "${ROOT_DIR}"/system/config_cncos_dns -d k8s-api.cncos.local
}

del_sysctl() {
  sed -ri.cncos-"$(date +%s)".bak '/cncos config begin/,/cncos config end/d' /etc/sysctl.conf
  sed -ri.cncos-"$(date +%s)".bak '/cncos config begin/,/cncos config end/d' /etc/security/limits.d/99-cncos.conf
}
