...
CACHE_DIR=${CACHE_DIR:-"./cncos-ops-offline"}
VERSION=
...
unMarshall_mainfest() {
  ...
  ver_num=$(yq e '.cncos-ops | length' "$manifest_file")
  local i=0
  while ((i < ver_num)); do
    IFS=',' read -ra projects <<<"$(yq -o csv e '.cncos-ops[0] | keys' "$manifest_file")"
    VERSION=$(yq e ".cncos-ops[$i].version" "$manifest_file")
    if [[ "$VERSION" == "$ver" ]] || [[ -z $ver ]]; then
      for project in "${projects[@]}"; do
        case $project in
          ...
          "bin-tools")
            ...
            yq e ".cncos-ops[$i].bin-tools" "$manifest_file" \
              | while IFS=': ' read -r p v; do "download_$p" "${v//\"/}"; done
            ;;
          "images")
            mkdir -pv "${CACHE_DIR}/version-${VERSION}/images"
            IFS=',' read -ra images <<<"$(yq -o csv e ".cncos-ops[$i].images" "$manifest_file")"
            download_img "${images[@]}"
            ;;
          *)
      ...
      tar cvzf "${CACHE_DIR}/cncos-ops-offline-${VERSION}.tgz" -C "${CACHE_DIR}" "version-${VERSION}/"
    else
